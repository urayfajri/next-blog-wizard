...
type Action = { type: 'SET_FIELD'; field: keyof DraftPost; value: string } | { type: 'NEXT' } | { type: 'BACK' } | { type: 'RESET' }

const initial: FormState = { title: '', author: '', summary: '', category: '', content: '', step: 0 }

function reducer(state: FormState, action: Action): FormState {
  switch(action.type) {
    case 'SET_FIELD': return { ...state, [action.field]: action.value } as FormState
...
export function BlogFormProvider({ children }: { children: React.ReactNode }){
  ...
  const set = (field: keyof DraftPost) => (value: string) => dispatch({ type: 'SET_FIELD', field, value })
